function dbQ2Y = Q2Y(obj,nComp)
%
% 用法：
%    [dbQ2Y,PRESSh,RESSh_1]= Q2Y(matX,matY,iComp,nFold)
%
% 功能：
...
if ~exist('nComp','var'), nComp=obj.nComp;end
...
for i=1:obj.nFold
    ...
    % 将第i个group的样本删除
    trainX = obj.matRawX(cvFoldIndex~=i,:);
    trainY = obj.matRawY(cvFoldIndex~=i,:);
    
    % 取出第i个group的样本
    testX = obj.matRawX(cvFoldIndex==i,:);

    T = ModelPLS(trainX,trainY);

    CompTopN(T,nComp,0);

    % 计算第i个group的样本的预测属性
    matBeta_ext = subGetBeta(trainX,trainY,T.matLoadingX,T.matLoadingY,T.matBasisX);

    preY(cvFoldIndex==i,:) = [ones(size(testX,1),1),testX] * matBeta_ext;
end

% 计算Y的预测误差平方和
PRESSh = sum(sum((obj.matRawY-preY).^2));

% 计算Y的预测误差平方和
RESSh_1 = sum(sum(obj.matRawY.^2));

% 计算交叉验证有效性
dbQ2Y = 1-PRESSh/RESSh_1;

clear testX trainX trainY cvFoldIndex  PRESSh PRESSh_1 i preY matBeta_ext T;
end
